...
using CryptoTools.Core.Api.Binance.Errors;
using CryptoTools.Core.Api.Binance.Implementations;
using CryptoTools.Core.Api.Binance.Interfaces;
using CryptoTools.Core.Api.Binance.Logging;
...
namespace Binance.Net
{
    ...
    public class BinanceClient: BinanceAbstractClient, IDisposable
    {
        ...
        private const string PriceEndpoint = "ticker/price";
        ...
        private const string ExchangeEndpoint = "exchangeInfo";
        ...
        {
            var result = await ExecuteRequest<BinanceExchangeInfo>(GetUrl(ExchangeEndpoint, Api, PublicVersion));
            return new BinanceApiResult<BinanceExchangeInfo>() { Success = result.Success, Data = result.Data, Error = result.Error };
        }
        ...
        public async Task<BinanceApiResult<BinancePrice>> GetTickerPriceAsync(string symbol)
        {
            if (AutoTimestamp && !timeSynced)
                await GetServerTimeAsync();

            var parameters = new Dictionary<string, string>() { { "symbol", symbol } };
            return await ExecuteRequest<BinancePrice>(GetUrl(PriceEndpoint, Api, "3", parameters));
        }
        ...
        /// <param name="receiveWindow">The receive window for which this request is active. When the request takes longer than this to complete the server will reject the request</param>
        /// <returns>List of open orders</returns>
        public async Task<BinanceApiResult<BinanceOrder[]>> GetOpenOrdersAsync(int? receiveWindow = null)
        {
            if (key == null || encryptor == null)
                return ThrowErrorMessage<BinanceOrder[]>(BinanceErrors.GetError(BinanceErrorKey.NoApiCredentialsProvided));
